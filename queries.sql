...
SELECT neutered, SUM(escape_attempts) FROM animals GROUP BY neutered HAVING SUM(escape_attempts) >= ALL(SELECT SUM(escape_attempts) FROM animals GROUP BY neutered);
